...
class OptionsViewModel @Inject constructor(
    ...
    fun applyNightMode(mode: Int) {
        application.applyNightMode(mode = mode)
        application.saveNightMode(mode = mode)
    }

    fun exportSubscriptions(directoryUri: Uri?) {
        directoryUri ?: return // User didn't choose any directory for export
        exportOnBackground(directoryUri = directoryUri)
    }

    fun importSubscriptions(contentUri: Uri?) {
        contentUri ?: return // User didn't choose any file for import

        // TODO: change temporary solution with path.contains(".json")
        if (contentUri.path?.contains(".json") == false) {
            return
        }
        importOnBackground(contentUri = contentUri)
    }
    ...
    private fun importOnBackground(contentUri: Uri) = viewModelScope.launch(ioDispatcher) {
        try {
            application.contentResolver.openInputStream(contentUri).use { inputStream ->
                ...
                val domainSubscriptions = importedSubscriptions.map { importedSubscription ->
